...
- `account` — identity, regions, tagged-resource sweep
- `org` — accounts, roots, OUs
- `ec2` — instances, security groups, VPCs, subnets, route tables, volumes, NAT gateways
- `elbv2` — load balancers, target groups, listeners, target health
- `autoscaling` — groups
- `eks` — clusters, node groups, add-ons, Fargate profiles
